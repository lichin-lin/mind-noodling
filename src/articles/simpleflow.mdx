...
export const metadata = {
  ...
  slug: "simpleflow",
  description: "From geometry algorithms to clipboard hacking: the story behind a Figma plugin",
...
<p className="time" dateTime="2025-11-01">November 1, 2025</p>
...
```JSON
TBD - from figma elements encodes into .fig(fig-kiwi hash)

this got insert into data-buffer
```

You can also test this yourself: try to create a new figma file, save it and upload it to [Evan Wallace’s .fig parser](https://madebyevan.com/figma/fig-file-parser/). You’ll see a miniature Figma file, complete with nodes and styles.
...
```JSON
TBD - demo some new connector styling
```
...
That’s how SimpleFlow’s **Copy Connector** button actually works, it assembles a valid element schema, encodes it the same way Figma does, save the data into Clipboard, and relies on Figma to handle the paste natively.
