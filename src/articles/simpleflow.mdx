...
### Encoding and Decoding using kiwi-schema

How does copying in Figma actually work? In short, Figma turns the selected elements (area) into a small `.fig` file through kiwi-schema format, encodes it in Base64, places that string inside the `data-buffer` attribute of an empty HTML `<span>` element, and then writes the entire HTML snippet to the clipboard as `text/html`.

You can test out the online version here as well: try to create a new figma file, save and upload it to [Evan Wallace’s .fig parser](https://madebyevan.com/figma/fig-file-parser/). You’ll see the actual contents, including Design data (.fig), Meta data (.json), and the thumbnail.
...
const figToJson = (fileBuffer: ArrayBuffer): object => {

  // Parse fig-kiwi format: [magic][delimiter][schema section][data section]
  ...
  // Encode blob bytes to base64 for JSON (if any)
  return convertBlobsToBase64(json);
}

// → { nodeChanges: [{ type: 'CONNECTOR', nodeGenerationData: {...}, ... }] }
const jsonData = figToJson(arrayBuffer);

// Now, jsonData for you to modify (Skip)
...
// After updating all the connector JSON data,

const jsonToFig = async (json: any, originalBuffer: ArrayBuffer): Promise<Uint8Array> => {
  
  // Parse fig-kiwi format: [magic][delimiter][schema section][data section]
  const [schemaByte, _] = figToBinaryParts(originalBuffer);
  
  // Encode JSON using original schema
  const schema = decodeBinarySchema(new ByteBuffer(schemaByte));
  const schemaHelper = compileSchema(schema);
